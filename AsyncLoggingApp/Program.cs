...
// Blocking HTTP call.
// Used to keep benchmark behavior identical to the sync version.
static void CallApiAndSleep(
HttpClient http,
    string url,
    string apiName,
    int iteration)
{
    try
    {
        string response = http.GetStringAsync(url)
            .GetAwaiter()
            .GetResult();
...
static void RunIterations(HttpClient http, Random rng, string chars, int count)
{
    for (int i = 0; i < count; i++)
    {
        ...
        Log.Error(
            ...
            orderItem.ProductId);
        // External API calls triggered 4 times across the full run.
        // HTTP behavior is intentionally identical to the sync benchmark.
        if (i == 999)
...
Console.WriteLine("[ASYNC] Running warm-up (50 iterations)...");
RunIterations(httpClient, random, Chars, WarmUpIterations);
Console.WriteLine("[ASYNC] Warm-up complete.\n");

// ── Timed section (5000 iterations) ──────────────────────────────────────────
var asyncStart = Stopwatch.GetTimestamp();
...
var asyncExecutionEnd = Stopwatch.GetTimestamp();

// ── Flush ──────────────────────────────────────────────────────────────────
var asyncFlushStart = Stopwatch.GetTimestamp();
Log.CloseAndFlush();
var asyncFlushEnd = Stopwatch.GetTimestamp();

long executionMs = (long)Stopwatch.GetElapsedTime(asyncStart, asyncExecutionEnd).TotalMilliseconds;
long flushMs     = (long)Stopwatch.GetElapsedTime(asyncFlushStart, asyncFlushEnd).TotalMilliseconds;
long totalMs     = (long)Stopwatch.GetElapsedTime(asyncStart, asyncFlushEnd).TotalMilliseconds;

Console.WriteLine();
Console.WriteLine("══════════════════════════════════════════════════════════════════════");
Console.WriteLine($"[ASYNC] Execution time (enqueue only) (ms):          {executionMs}");
Console.WriteLine($"[ASYNC] Flush duration (ms):                         {flushMs}");
Console.WriteLine($"[ASYNC] Total real time (execution + flush) (ms):    {totalMs}");
Console.WriteLine("══════════════════════════════════════════════════════════════════════");
Console.WriteLine();
Console.WriteLine("NOTE: Async execution time only reflects the cost of enqueuing log");
Console.WriteLine("      events into the in-memory buffer. Actual file I/O happens on a");
Console.WriteLine("      background thread and is captured in the flush duration above.");
